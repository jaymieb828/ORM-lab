...
def problem_one(request):
  ...
  student_greater_3 = Student.objects.filter(gpa__gt=3.0).order_by("-gpa")

  for student in student_greater_3:
...
def problem_two(request):
  ...
  instructors = Instructor.objects.filter(hire_date__year__gte = 2010).order_by('hire_date')
...
"""
will actually only pull up one person
...
def problem_three(request):
  instructor_pk2 = Instructor.objects.get(id =2)
  courses_pk2 = Course.objects.filter(instructor_id = 2)

  print(f"Instructor Name: {instructor_pk2.first_name} {instructor_pk2.last_name}\nCourses:")

  for course in courses_pk2:
    print(f"\t- {course.name}")
...
def problem_four(request):
  ...
  print(f" Student Counts = {student_count}")
  print(f" Courses Counts = {course_count}")
  print(f" Instructor Counts = {instructor_count}")
...
def problem_five(request):
  ...
  student = Student.objects.create(first_name = "Jaymie", last_name = "Batoon", year = 2022, gpa = 2.5)
...
def bonus_problem(request):
  instructor = Instructor.objects.filter(course)
